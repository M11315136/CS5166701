import { _decorator, Component, Node, Animation, Prefab, Vec3, instantiate } from 'cc';
...
export class RemoteEntityView extends Component {
    ...
    private _createGhost(clientId: string): RemotePlayerEntry {
        ...
        const anim = ghost.getComponent(Animation) ?? ghost.getComponentInChildren(Animation);
    ...
    private _playAnim(anim: Animation | null, animState: AnimState): void {
        if (!anim) return;
        const name =
            animState === 'idle' ? this.idleAnim :
            animState === 'run'  ? this.walkAnim : this.jumpAnim;
        anim.play(name);
    }
}
