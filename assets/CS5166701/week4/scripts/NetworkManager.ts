...
export class NetworkManager extends Component {
    ...
    public static get instance(): NetworkManager | null { return NetworkManager._instance; }
    ...
    // ── Callbacks（由 GameContextWeek4 設定）─────────────────────────────────
    /** 收到其他玩家位置/動畫狀態時呼叫 */
    ...
    /** 排行榜有更新時呼叫 */
    public onLeaderboardUpdate: ((data: LeaderboardEntry[]) => void) | null = null;
    ...
    private _connect(): void {
        ...
        this._ws.onerror = () => {
            console.error('[Net] WebSocket 發生錯誤，請確認伺服器已啟動（node server.js）');
        };
    }
    ...
    /** 發送本地玩家的位置與動畫狀態 */
    public sendPlayerState(x: number, y: number, scaleX: number, animState: AnimState): void {
        this._send({
            type: NetMsgType.PlayerState,
            clientId: this._clientId,
            x, y, scaleX, animState,
        });
    }

    /** 發送怪物的位置與動畫狀態（僅主控端呼叫） */
    public sendMonsterState(x: number, y: number, scaleX: number, animState: AnimState): void {
